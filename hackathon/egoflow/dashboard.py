...
def render_dashboard(
    ...
    *,
    ...
    event_total = sum(len(rows) for rows in events_by_episode.values())
    ...
    for episode_id, task, duration, completion, video_path in episodes:
        ...
        event_marks = "".join(
            ...
            f'data-end="{end:.3f}" data-title="{html.escape(label.replace("_", " ").title(), quote=True)} · {html.escape(episode_id, quote=True)}"></span>'
        ...
        <article class="episode" data-testid="episode-card" data-id="{html.escape(episode_id, quote=True)}" data-task="{html.escape(task.replace('_',' '), quote=True)}" data-search="{html.escape((episode_id+' '+task).lower())}" data-flags="{' '.join(flags)}">
          <header class="episode-head">
            ...
            <div class="episode-actions"><button class="shortlist" data-action="shortlist" aria-pressed="false">＋ Add to demo</button><div class="score"><b>{100*(completion or 0):.1f}%</b><span>completion score</span></div></div>
          </header>
...
<div class="topline"><span class="brand">EGOFLOW</span><span class="version">Demonstration review · frozen baseline + learned v2</span></div>
...
<section class="stats"><div class="stat"><b>{len(episodes)}</b><span>episodes</span></div><div class="stat"><b>{total_seconds/60:.1f} min</b><span>indexed video</span></div><div class="stat"><b>{event_total}</b><span>all signal windows</span></div><div class="stat"><b>{manual_total}</b><span>human labels</span></div><div class="stat"><b>{clean_total/60:.1f} min</b><span>candidate-free spans ≥10s</span></div></section>
<section class="hero"><div class="hero-media">{hero_media}</div><aside class="hero-copy"><p class="overline">Validation example · organizing plushies</p><h2>A subtle action switch, not a long stall.</h2><p>The human review span is 8–13 seconds. The new expected-dynamics model identifies an unusual two-hand transition inside it.</p><div class="result"><b>{learned_event.get('start_sec','—')}–{learned_event.get('end_sec','—')} sec</b><span>{learned_event.get('surprise_mad','—')} MAD above expected hand dynamics · confidence {learned_event.get('confidence','—')}</span></div><p>The earlier geometric baseline lands at {hand_event.get('start_sec','—')}–{hand_event.get('end_sec','—')} seconds. Neither signal claims to read intent or identify the target object.</p><div class="legend"><span class="key learned-v2">Learned v2</span><span class="key hybrid">Hybrid progress + visual</span><span class="key aux">Auxiliary visual</span><span class="key hand">Hand geometry</span><span class="key human">Human label</span></div></aside></section>
...
<section><div class="catalog-head"><div><p class="overline">Episode database</p><h2>Inspect the evidence without crowding the graph.</h2></div><div class="controls"><input id="search" data-testid="episode-search" aria-label="Search episodes" placeholder="Search task or episode ID"><label for="layer">Show evidence</label><select id="layer" data-testid="evidence-filter"><option value="core">Core review signals</option><option value="learned-v2">Learned v2 only</option><option value="hybrid">Hybrid only</option><option value="aux">Auxiliary only</option><option value="hand">Hand geometry only</option><option value="human">Human labels only</option><option value="all">All evidence</option></select><button id="shortlist-toggle" class="demo-toggle" data-testid="shortlist-toggle" aria-pressed="false">Demo reel · <span id="shortlist-count">0</span></button><button id="shortlist-clear" class="clear-shortlist">Clear</button></div></div><p class="layer-note" id="layer-note">Core view: learned-v2, hybrid, and human windows. Auxiliary and geometric prototypes are available from the menu.</p><div class="demo-rail" id="demo-rail"><span class="demo-rail-label">Your demo reel</span><span class="demo-empty">Add the strongest episodes; this shortlist stays saved in your browser.</span></div><div class="episodes" id="episodes">{''.join(cards)}</div></section>
...
const shortlistToggle=document.querySelector('#shortlist-toggle'), shortlistClear=document.querySelector('#shortlist-clear'), shortlistCount=document.querySelector('#shortlist-count'), demoRail=document.querySelector('#demo-rail');
...
const storageKey='egoflow-demo-shortlist-v1'; let shortlist=new Set(JSON.parse(localStorage.getItem(storageKey)||'[]')), shortlistOnly=false, stopAt=null;
const notes={{core:'Core view: learned-v2, hybrid, and human windows. Auxiliary and geometric prototypes are available from the menu.','learned-v2':'Only self-supervised hand-dynamics deviations are shown.',hybrid:'Only events combining learned progress and frozen visual evidence are shown.',aux:'Only the disclosed frozen-visual heuristic layer is shown.',hand:'Only the geometric hand-redirection prototype is shown.',human:'Only manually labeled intervals are shown.',all:'All stored evidence layers are visible.'}};
function renderShortlist(){{shortlistCount.textContent=shortlist.size; cards.forEach(card=>{{const selected=shortlist.has(card.dataset.id), button=card.querySelector('.shortlist'); card.classList.toggle('shortlisted',selected); button.setAttribute('aria-pressed',selected); button.textContent=selected?'✓ In demo reel':'＋ Add to demo'}}); const selected=cards.filter(card=>shortlist.has(card.dataset.id)); demoRail.innerHTML='<span class="demo-rail-label">Your demo reel</span>'+(selected.length?selected.map(card=>`<button class="demo-chip" data-jump="${{card.dataset.id}}">${{card.dataset.task}} · ${{card.dataset.id.slice(0,6)}}</button>`).join(''):'<span class="demo-empty">Add the strongest episodes; this shortlist stays saved in your browser.</span>'); localStorage.setItem(storageKey,JSON.stringify([...shortlist])); apply()}}
function apply(){{const query=search.value.toLowerCase(), selected=layer.value; cards.forEach(card=>card.hidden=!card.dataset.search.includes(query)||(shortlistOnly&&!shortlist.has(card.dataset.id))); document.querySelectorAll('[data-source]').forEach(mark=>{{const source=mark.dataset.source; mark.hidden=selected==='core'?!['learned-v2','hybrid','human','learned'].includes(source):selected!=='all'&&source!==selected}}); note.textContent=notes[selected]}}
function openViewer(button){{const start=Number(button.dataset.start||0), end=Number(button.dataset.end||0); stopAt=end>start?end:null; viewerTitle.textContent=button.dataset.title||'Episode'; viewerMeta.textContent=stopAt?`Review window · ${{start.toFixed(2)}}–${{end.toFixed(2)}} seconds`:`Full episode · starting at ${{start.toFixed(2)}} seconds`; viewerVideo.src=button.dataset.video; viewerVideo.addEventListener('loadedmetadata',()=>{{viewerVideo.currentTime=Math.min(start,Math.max(0,viewerVideo.duration-.05)); viewerVideo.play().catch(()=>{{}})}},{{once:true}}); viewer.showModal()}}
document.addEventListener('click',event=>{{const action=event.target.closest('[data-action]'); if(action?.dataset.action==='shortlist'){{const id=action.closest('.episode').dataset.id; shortlist.has(id)?shortlist.delete(id):shortlist.add(id); renderShortlist()}} else if(action?.dataset.action==='watch') openViewer(action); const jump=event.target.closest('[data-jump]'); if(jump){{shortlistOnly=false; shortlistToggle.classList.remove('active'); shortlistToggle.setAttribute('aria-pressed','false'); apply(); const card=cards.find(item=>item.dataset.id===jump.dataset.jump); card?.scrollIntoView({{behavior:'smooth',block:'center'}})}}}});
...
shortlistToggle.addEventListener('click',()=>{{shortlistOnly=!shortlistOnly; shortlistToggle.classList.toggle('active',shortlistOnly); shortlistToggle.setAttribute('aria-pressed',String(shortlistOnly)); apply()}}); shortlistClear.addEventListener('click',()=>{{shortlist.clear(); shortlistOnly=false; shortlistToggle.classList.remove('active'); shortlistToggle.setAttribute('aria-pressed','false'); renderShortlist()}});
search.addEventListener('input',apply); layer.addEventListener('change',apply); renderShortlist();
