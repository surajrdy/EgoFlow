...
def render_dashboard(
    ...
    *,
    ...
    for episode_id, task, duration, completion, video_path in episodes:
        ...
        events = [event for event in events_by_episode[episode_id] if event[3] == "LEARNED V2"]
...
<section><div class="catalog-head"><div><p class="overline">Episode database</p><h2>Inspect the evidence without crowding the graph.</h2></div><div class="controls"><button id="review-next" data-testid="review-next">Review strongest →</button><input id="search" data-testid="episode-search" aria-label="Search episodes" placeholder="Search task or episode ID"><label for="layer">Show evidence</label><select id="layer" data-testid="evidence-filter"><option value="demo">Learned + reviewed</option><option value="learned-v2">Learned only</option><option value="human">Reviewed spans only</option></select><button id="shortlist-toggle" class="demo-toggle" data-testid="shortlist-toggle" aria-pressed="false">Demo reel · <span id="shortlist-count">0</span></button><button id="shortlist-clear" class="clear-shortlist">Clear</button></div></div><p class="layer-note" id="layer-note">Learned interaction candidates and reviewed spans. Click a timeline mark, or use “Review strongest” to step through the highest-confidence candidates.</p><div class="demo-rail" id="demo-rail"><span class="demo-rail-label">Your demo reel</span><span class="demo-empty">Add the strongest episodes; this shortlist stays saved in your browser.</span></div><div class="episodes" id="episodes">{''.join(cards)}</div></section>
...
</main><dialog id="viewer" class="viewer"><div class="viewer-head"><div><strong id="viewer-title">Episode</strong><p id="viewer-meta">Starting at 0 seconds</p></div><div><button id="viewer-keep">Keep in demo</button><button id="viewer-next">Next candidate</button><button id="viewer-close" class="viewer-close" aria-label="Close video">Close</button></div></div><video id="viewer-video" controls playsinline preload="metadata"></video></dialog><script>
...
const storageKey='egoflow-demo-shortlist-v1'; let shortlist=new Set(JSON.parse(localStorage.getItem(storageKey)||'[]')), shortlistOnly=false, stopAt=null, currentEpisode=null, reviewCursor=-1;
const notes={{demo:'Learned interaction candidates and reviewed spans.','learned-v2':'Only self-supervised hand-dynamics candidates are shown.',human:'Only reviewed spans are shown.'}};
...
function apply(){{const query=search.value.toLowerCase(), selected=layer.value; cards.forEach(card=>card.hidden=!card.dataset.search.includes(query)||(shortlistOnly&&!shortlist.has(card.dataset.id))); document.querySelectorAll('[data-source]').forEach(mark=>{{const source=mark.dataset.source; mark.hidden=selected==='demo'?!['learned-v2','human'].includes(source):source!==selected}}); note.textContent=notes[selected]}}
function rankedCandidates(){{return [...document.querySelectorAll('.event-mark.learned-v2,.event-mark.hybrid')].sort((a,b)=>Number(b.dataset.confidence||0)-Number(a.dataset.confidence||0))}}
function openViewer(button){{const start=Number(button.dataset.start||0), end=Number(button.dataset.end||0); stopAt=end>start?end:null; currentEpisode=button.dataset.episode||button.closest('.episode')?.dataset.id||null; viewerTitle.textContent=button.dataset.title||'Episode'; viewerMeta.textContent=stopAt?`Review window · ${{start.toFixed(2)}}–${{end.toFixed(2)}} seconds`:`Full episode · starting at ${{start.toFixed(2)}} seconds`; viewerVideo.src=button.dataset.video; viewerVideo.addEventListener('loadedmetadata',()=>{{viewerVideo.currentTime=Math.min(start,Math.max(0,viewerVideo.duration-.05)); viewerVideo.play().catch(()=>{{}})}},{{once:true}}); if(!viewer.open)viewer.showModal()}}
function reviewNext(){{const candidates=rankedCandidates(); if(!candidates.length)return; reviewCursor=(reviewCursor+1)%candidates.length; openViewer(candidates[reviewCursor])}}
...
document.querySelector('#review-next').addEventListener('click',reviewNext); document.querySelector('#viewer-next').addEventListener('click',reviewNext); document.querySelector('#viewer-keep').addEventListener('click',()=>{{if(currentEpisode){{shortlist.add(currentEpisode); renderShortlist()}} reviewNext()}});
