...
<section><div class="catalog-head"><div><p class="overline">Featured episodes</p><h2>Learned interaction signals across demonstrations.</h2></div><div class="controls"><input id="search" data-testid="episode-search" aria-label="Search episodes" placeholder="Search task or episode ID"><label for="layer">Show evidence</label><select id="layer" data-testid="evidence-filter"><option value="demo">Learned + reviewed</option><option value="learned-v2">Learned only</option><option value="human">Reviewed spans only</option></select></div></div><p class="layer-note" id="layer-note">Learned interaction candidates and reviewed spans. Click any timeline mark to inspect that moment.</p><div class="episodes" id="episodes">{''.join(cards)}</div></section>
...
function apply(){{const query=search.value.toLowerCase(), selected=layer.value; cards.forEach(card=>card.hidden=!card.dataset.search.includes(query)); document.querySelectorAll('[data-source]').forEach(mark=>{{const source=mark.dataset.source; mark.hidden=selected==='demo'?!['learned-v2','human'].includes(source):source!==selected}}); note.textContent=notes[selected]}}
